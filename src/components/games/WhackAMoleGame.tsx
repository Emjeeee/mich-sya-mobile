  ...
  return (
    <GameCard>
      <View style={styles.headerRow}>
        <Text style={styles.muted}>{running ? `Sisa waktu: ${timeLeft}s` : 'Ketuk mol yang muncul'}</Text>
        <Text style={styles.score}>{score}</Text>
      </View>

      <View style={styles.grid}>
        {Array.from({ length: HOLE_COUNT }, (_, i) => (
          <Pressable key={i} onPress={() => whack(i)} style={styles.hole}>
      ...
      <View style={styles.center}>
        ...
        {!running && timeLeft === 0 && (
          <>
            <Text style={styles.resultText}>Waktu habis — skor {score}</Text>
